...
- Replace name-only definition inference with scoped resolution and exact ID/version pins where a definition is actually used. Preserve old findings. Offer a bounded review list for frequently reused legacy records; validated enrichment uses the existing supersession/acceptance procedure and downstream impact review. No mass auto-backfill from titles or record membership.

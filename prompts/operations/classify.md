...
- **An `investigation` restated is not a new investigation.** Analytical questions get reworded every time someone asks them again — "Analyze live paywall variant performance" and "Analyze paywall variant performance live" are one piece of work. Before proposing `kind: "investigation"`, read every candidate investigation's title *and goal*: if one pursues the same question in different words, use its id. Proposing a twin is the failure that turns one question into a pile of open records nobody binds to.
- **This operation never opens an investigation.** An investigation is identified by its question, which a session declares (`ledger_investigation_new`) or binds to (`ledger_investigation_bind`); the folder the chat ran in is not its identity, and an investigation with no repo is complete. A `new_record` of kind `investigation` that matches no open investigation is declined and its span reported unassigned with that reason, so propose one only to point at work you believe is already open under other words; otherwise leave analysis spans unassigned. Other kinds (`implementation`, `writing`, `decision`, `other`) may still be proposed as new records.

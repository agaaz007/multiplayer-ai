...
export async function usageSummary(pool:pg.Pool,opts:{from:string;to:string;actor?:string;traffic_class?:string}) {
  ...
  return withoutUsage(async () => {
    ...
    const invocations=await pool.query(`select i.actor,i.tool,i.purpose,i.traffic_class,i.outcome,i.availability,count(*)::int as invocations,count(i.finished_at)::int as terminal_outcomes,avg(i.duration_ms) as mean_duration_ms,percentile_cont(0.95) within group(order by i.duration_ms) as p95_duration_ms,percentile_cont(0.99) within group(order by i.duration_ms) as p99_duration_ms,count(*) filter(where i.logical_operation_key is null)::int as invocations_without_retry_identity from cont_usage_invocations i where ${where} group by 1,2,3,4,5,6 order by 1,2,3,4,5,6`,params);
    // Logical totals deliberately omit outcome/availability: a failed attempt followed
    // by success is one known operation, while both dispatches remain in invocations.
    const logical=await pool.query(`select i.actor,i.tool,i.purpose,i.traffic_class,count(*)::int as invocation_attempts,count(distinct i.logical_operation_key)::int as known_logical_operations,count(*) filter(where i.logical_operation_key is not null)::int-count(distinct i.logical_operation_key)::int as known_retry_attempts,count(*) filter(where i.logical_operation_key is null)::int as invocations_without_retry_identity,count(distinct i.logical_operation_key) filter(where i.outcome='success')::int as known_operations_with_success from cont_usage_invocations i where ${where} group by 1,2,3,4 order by 1,2,3,4`,params);
    ...
    return {retention_history:retention.rows,window:{from:opts.from,to:opts.to,bounds:"[from,to)"},invocations:invocations.rows,logical_operations:logical.rows,storage_operations:operations.rows,agent_reported_references:references.rows,operations_without_invocation:orphaned.rows[0].operations_without_invocation,local_spool:await usageSpoolHealth(),limitations:["Unknown traffic remains unknown; no historical backfill is inferred.","Invocations are grouped by invocation start; SQL dispatches by operation start. Retry grouping is known only when a caller supplied an idempotency key.","Logical-operation totals are separate from outcome groups; invocations without retry identity cannot be deduplicated.","Reference appearances are agent-reported use, not completed handoffs.","Local spool health covers this machine; remote machine completeness requires its health evidence."]};
  });
}

...
export async function latestVerifiedSnapshot(pool: pg.Pool, opts: {threadId?: string; sessionIds?: string[]; repos?: string[]; asOf?: Date | null}): Promise<VerifiedSnapshot | null> {
  if (opts.sessionIds && !opts.sessionIds.length || opts.repos && !opts.repos.length) return null;
  ...
  const row = (await pool.query<CheckpointRow & {repo:string}>(`select c.*, t.repo from cont_checkpoints c join cont_threads t on t.id=c.thread_id
    where ${conditions.join(' and ')} order by c.verified_snapshot_at desc,c.created_at desc,c.id desc limit 1`,params)).rows[0];
  return row ? {status:'verified',checkpoint_id:row.id,thread_id:row.thread_id,session_id:row.session_id,repo:row.repo,
    ref:row.wip_ref!,commit:row.wip_commit!,base_commit:row.base_commit,verified_at:row.verified_snapshot_at!,
    through_event_seq:row.through_event_seq,created_at:row.created_at} : null;
}

...
let recLatency: import("./continuity/records.js").WorkRecord;
{
  ...
  assert.equal(ra.candidates, 2, "banner (repo) + launch email (non-code)");
  ...
  assert.ok(p.includes("# Candidate records (2)"));
  assert.ok(p.includes(`[1] ${recBanner.id} · kind: implementation · title: Greeting banner · repo: ${REPO}`), "candidate line has id/kind/title/repo");
  ...
  assert.ok(p.includes(`[2] ${recEmail.id} · kind: writing · title: Launch email · repo: none (non-code work)`));
  assert.ok(p.includes("state: no updates yet"));
  for (const r of [recOther, recOld, recDone, recSearch]) assert.ok(!p.includes(r.id), `${r.title} is not a candidate`);
...
// ---------- (d) idempotent re-runs ----------
{
  ...
  assert.equal(r2.candidates, 3, "the new record is now a candidate (linked to this session)");
...
// ---------- (b) invalid output: each bad item rejected with a reason; only valid items written ----------
{
  ...
  assert.equal(rb.candidates, 3, "banner, search (repo) + launch email; latency record too");

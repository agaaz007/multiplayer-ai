...
export function captureStaleness(hb: HelperHeartbeat | null, now: Date, opts: { alive?: (pid: number) => boolean; staleMs?: number } = {}): string | null {
  ...
  if (!inPass && Number.isFinite(since) && t - since > staleMs) return `Ledger capture on this machine has not completed a pass for ${ago(t - since)}; new work is not captured.${lastError} ${HELPER_RESTART_HINT}`;
  if (hb.last_error) return `Ledger capture on this machine reported errors in its last pass: ${hb.last_error.slice(0, 200)}`;
  return null;
}

/** The promise's value, or timedOut after ms. The timer never keeps the process alive. */
export async function withDeadline<T>(p: Promise<T>, ms: number): Promise<{ timedOut: false; value: T } | { timedOut: true }> {
  let timer: NodeJS.Timeout | undefined;
  const deadline = new Promise<{ timedOut: true }>((res) => { timer = setTimeout(() => res({ timedOut: true }), ms); timer.unref?.(); });
  try {

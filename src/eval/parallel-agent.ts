...
function promptFor(phase: ParallelTurn["phase"]): string {
  ...
  if (phase === "before") return scope + "Implement module.exports = function aggregate(events) in attribution.cjs. Deduplicate purchases by id (first occurrence wins), group by channel in first-seen order, and return an object whose values are {purchases: count, revenue: sum of amount}. Run exactly `node check-attribution.cjs before`. The word `before` is the required literal CLI argument for every validation attempt in this turn, including after you implement or fix the function. If the command fails, fix attribution.cjs and rerun the same `node check-attribution.cjs before` command until it exits successfully. Do not use the `after` phase until a later instruction requests it. End with the successful command result.";
  if (phase === "during") return scope + "Keep validating attribution while another agent continues its paywall task. Execute exactly `node check-attribution.cjs during` with a shell timeout of at least 750000 ms. The word `during` is the required literal CLI argument. This command performs independent varied-input checks every 200 ms until the controller creates attribution-stop. Wait for this exact command to finish successfully; do not create attribution-stop yourself and do not background the command. If the shell yields, poll that same process until it finishes. End with its result.";
  return scope + "The handoff is over. Run exactly `node check-attribution.cjs after` to independently confirm your attribution code still works. The word `after` is the required literal CLI argument. Do not change any file manually in this turn. End with the command result.";
}

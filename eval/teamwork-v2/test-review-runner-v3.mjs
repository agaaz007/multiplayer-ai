...
import { run, preflight, validateReview, disagreement } from './review-runner-v3.mjs';
...
function fixture(t) {
  ...
  const cfg={schema:'pm-review-run/v3',execution_authorized:true,authorization:'fake-session validation only',runtime,runtime_sha256:hash(fs.readFileSync(path.join(runtime,'eval/sequence-codex.js'))),
...
test('v3 explicitly declares critical items and supplies an arm-blind public self-check',async t=>{
  const {config,out}=fixture(t);
  const result=await run(config,out,{sessionRunner:async input=>{
    const format=JSON.parse(fs.readFileSync(path.join(input.worktree,'review-format.json'),'utf8'));
    assert.deepEqual(Object.keys(format.critical_errors[0]),['kind','explanation','evidence']);
    assert.equal(typeof format.residual_identification[0],'string');
    assert.ok(input.prompt.includes('node validate-review.mjs'));
    const validator=fs.readFileSync(path.join(input.worktree,'validate-review.mjs'),'utf8');
    assert.ok(!/\b(?:ledger|supermemory|mem0|gbrain|graphify)\b|\/Users\/|controller-map|expected_facts/i.test(validator));
    save(path.join(input.worktree,'review.json'),judgment());
    const selfCheck=spawnSync(process.execPath,['validate-review.mjs'],{cwd:input.worktree,encoding:'utf8'});
    assert.equal(selfCheck.status,0,selfCheck.stderr);assert.equal(JSON.parse(selfCheck.stdout).valid,true);return processResult;
  }});
  assert.equal(result.cases['case-0001'].status,'reviewed_agreement');
  const manifest=JSON.parse(fs.readFileSync(path.join(out,'manifest.json'),'utf8'));
  assert.match(manifest.validator_sha256,/^[a-f0-9]{64}$/);
});

test('standalone public validator and controller agree on exact evidence and typed claims',async()=>{
  const {validateReview:publicValidate}=await import('./review-validator-v3.mjs');
  for(const mutate of [r=>r,r=>{r.dimensions.synthesis.evidence[0].quote='Paraphrased pause';return r;},r=>{r.factual_assessment.guardrail_breached.answer_evidence[0].observed='true';return r;},r=>{r.critical_errors=[{type:'wrong',rationale:'Missing canonical fields',evidence:[{source:'answer',quote:answer.recommendation}]}];return r;}]){
    const r=mutate(judgment());assert.deepEqual(publicValidate(r,answer,sources),validateReview(r,answer,sources));
  }
});

  ...
  function walk(from,to,relative='') {
    for(const entry of fs.readdirSync(from,{withFileTypes:true}).sort((a,b)=>a.name.localeCompare(b.name))) {
      ...
      if(entry.isDirectory()){fs.mkdirSync(output);walk(input,output,name);}
        ...
        fs.writeFileSync(output,content,{mode:0o600});files[name]=hash(content);
      }
    }
  }
...
function validateRequest(req) {
  ...
  if(output.startsWith(workspace+path.sep)||output.startsWith(home+path.sep))throw new Error('controller output is visible inside agent roots');
...
async function run(requestFile) {
  ...
  if(profile.readiness_verified!==true || profile.paid_paths_gated!==true)throw new Error('native readiness or paid-path budget gate not verified');
  ...
  const mcp={};const started=monotonic();const output=req.controller_output_dir;
  ...
  try {
    ...
    save(path.join(output,'stage-result.json'),{schema:'teamwork-stage/v2',arm:req.arm,track:req.track,stage:req.stage,
      ...
      elapsed_ms:elapsed,timed_out:result.timedOut,timing_valid:elapsed<=req.stage_deadline_ms+5000,

...
function verifyEvidence(cfg){
 if(!cfg.readiness_receipt||!fs.existsSync(cfg.readiness_receipt))return false;
...
export async function provision(cfg){
 ...
 if(cfg.arm==='ledger'){
  ...
  const database='gbrain_native_v3_'+crypto.randomBytes(12).toString('hex'),role=database+'_role',nonce=crypto.randomBytes(24).toString('hex');
  const adminUrl=`postgresql://${encodeURIComponent(os.userInfo().username)}@127.0.0.1:5432/postgres`;const url=new URL(adminUrl);url.pathname='/'+database;url.username=role;
  const client=new pg.Client({connectionString:adminUrl});await client.connect();
  try{await client.query(`create role "${role}" login nosuperuser nocreatedb nocreaterole bypassrls`);await client.query(`create database "${database}" owner "${role}"`);await client.query(`comment on database "${database}" is '${nonce}'`);}finally{await client.end();}
  const extensionUrl=new URL(adminUrl);extensionUrl.pathname='/'+database;const extensions=new pg.Client({connectionString:extensionUrl.toString()});await extensions.connect();try{await extensions.query('create extension if not exists vector');}finally{await extensions.end();}
  state.database_name=database;state.database_role=role;state.database_nonce=nonce;state.database_url=url.toString();state.admin_url=adminUrl;write(statePath(cfg),state);
  const {prepareNativePostgresPrereqs}=await import('./native-gbrain-upgrade.mjs');await prepareNativePostgresPrereqs(cfg,state);
  gbrain(cfg,['init','--url',state.database_url,'--non-interactive','--embedding-model','openai:text-embedding-3-large','--embedding-dimensions','3072','--expansion-model','openai:gpt-4.1-mini','--chat-model','openai:gpt-4.1-mini','--skip-embed-check']);const c=read(path.join(home,'.gbrain','config.json'));
  if(c.engine!=='postgres'||c.database_url!==state.database_url)throw new Error('GBrain native PostgreSQL configuration mismatch');
  c.storage={backend:'local',bucket:namespace,localPath:path.join(home,'attachments')};fs.mkdirSync(c.storage.localPath,{mode:0o700});write(path.join(home,'.gbrain','config.json'),c);
  for(const [key,value]of Object.entries({'models.default':'openai:gpt-4.1-mini','models.expansion':'openai:gpt-4.1-mini','facts.extraction_model':'openai:gpt-4.1-mini','facts.default_visibility':'world','search.mode':'balanced','search.expansion':'true','search.reranker.enabled':'true','search.reranker.model':'llama-server-reranker:qwen3-reranker-4b','search.reranker.timeout_ms':'30000'}))gbrain(cfg,['config','set',key,value]);
 }else if(cfg.arm==='graphify'){
  fs.mkdirSync(path.join(cfg.root,'graphify'),{mode:0o700});
...
export async function prepareStage(req){
 ...
 if(cfg.arm==='ledger'){
  ...
  p.mcp.gbrain={command:cfg.gbrain_binary??'gbrain',args:['serve','--surface','full'],env:env(path.join(cfg.root,'gbrain-home'),cfg.provider_env),cwd:req.workspace};
  const conf=path.join(req.controller_output_dir,'gbrain-config.json');write(conf,{...cfg,workspace:req.workspace,expected_database_url:state.database_url,code_source_id:'default'});
  p.mcp.gbrain_files={command:process.execPath,args:[path.join(here,'native-gbrain.mjs'),conf],env:env(req.controller_output_dir,cfg.provider_env),cwd:req.workspace};
  p.allow_local_postgres=true;p.read_paths.push(path.join(cfg.root,'gbrain-home'));p.write_paths.push(path.join(cfg.root,'gbrain-home'));
 }else if(cfg.arm==='graphify'){
  ...
  const conf=path.join(req.controller_output_dir,'graphify-config.json');write(conf,{...cfg,workspace:req.workspace,namespace:state.namespace});
  p.mcp.graphify={command:process.execPath,args:[path.join(here,'native-graphify.mjs'),conf],env:env(req.controller_output_dir,cfg.provider_env),cwd:req.workspace};
  p.mcp.graphify_native={command:path.join(path.dirname(cfg.graphify.binary),'graphify-mcp'),args:['--graph',path.join(cfg.root,'graphify','graphify-out','graph.json')],env:env(path.join(cfg.root,'graphify','home')),cwd:path.join(cfg.root,'graphify')};
  p.read_paths.push(path.join(cfg.root,'graphify'));p.write_paths.push(path.join(cfg.root,'graphify'));
 }else if(cfg.arm==='supermemory'){
  const api=await load(cfg,'analytical-native');const codexHome=path.join(req.fresh_home,'.codex');api.copyFrozenDirectory(cfg.supermemory_template,codexHome);write(path.join(codexHome,'supermemory.json'),{projectContainerTag:state.namespace,userContainerTag:state.namespace,recallMode:'direct',baseUrl:cfg.provider_env?.SUPERMEMORY_API_URL});
...
export async function captureStage(req){
 ...
 if(cfg.arm==='ledger'){
  ...
  const {nativeGbrain}=await import('./native-gbrain.mjs');const native=nativeGbrain({...cfg,workspace:req.workspace,transcript_root:path.join(req.fresh_home,'.codex','sessions')});
  result.artifacts=native.capture(req.stage);result.health=native.operation('get_health',{});result.capture_mode='native full pages and exact attachment bytes; no semantic controller summary';
 }else if(cfg.arm==='graphify'){
  const {nativeGraphify}=await import('./native-graphify.mjs');const native=nativeGraphify({...cfg,workspace:req.workspace,transcript_root:path.join(req.fresh_home,'.codex','sessions')});
  result.artifacts=native.capture(req.stage);result.extraction=native.extract(false);result.capture_mode='complete original corpus and official AST+semantic extraction';result.processing_complete=true;
 }else if(cfg.arm==='supermemory'){
  const {nativeSupermemory}=await import('./native-supermemory.mjs');
  result.artifacts=await nativeSupermemory({...cfg,namespace:state.namespace,workspace:req.workspace,transcript_root:path.join(req.fresh_home,'.codex/sessions')}).capture(req.stage);
...
export async function exportSequence(cfg){
 ...
 if(cfg.arm==='gbrain'){const dir=path.join(cfg.root,'gbrain-export');gbrain(cfg,['export','--dir',dir]);out.directory=dir;
  const state=read(statePath(cfg)),db=new pg.Client({connectionString:state.database_url});await db.connect();let files;try{files=(await db.query('select page_slug,filename,storage_path,size_bytes,content_hash from files order by storage_path')).rows;}finally{await db.end();}
  const {nativeGbrain}=await import('./native-gbrain.mjs'),native=nativeGbrain({...cfg,workspace:cfg.root});for(const file of files){const data=native.bytes(file.storage_path);if(crypto.createHash('sha256').update(data).digest('hex')!==file.content_hash)throw new Error('native export attachment hash mismatch');}
...
export async function cleanupSequence(cfg){
 ...
 if(cfg.arm==='gbrain'){const state=read(statePath(cfg));const db=new pg.Client({connectionString:state.admin_url});await db.connect();try{const row=(await db.query("select shobj_description(oid, 'pg_database') as nonce from pg_database where datname=$1",[state.database_name])).rows[0];if(row?.nonce!==state.database_nonce)throw new Error('GBrain DB ownership mismatch');if(!/^gbrain_native_v3_[a-f0-9]{24}$/.test(state.database_name))throw new Error('invalid owned database');await db.query(`drop database "${state.database_name}"`);if(state.database_role===state.database_name+'_role')await db.query(`drop role "${state.database_role}"`);}finally{await db.end();}}
 if(cfg.arm==='supermemory'){
